...
- **immuni_otp**. This contains the logic of the specific backend service.
tests. This contains the unit tests.

The *immuni_otp* folder is grouped by concerns. Specifically, as follows:
...
- **models**. This contains the models (e.g., dataclasses, [marshmallow](https://pypi.org/project/marshmallow/)) used by the backend service.

### Microservices
The backend service logic runs microservices, each one identified by a dedicated file within the *immuni_otp* folder. Specifically, as follows:
...
The Immuni project is composed of different repositories—one for each component or service. If you wish to raise something strictly relevant to this repository (i.e., an OTP Service bug), please read on. However, to raise other issues or to highlight other bugs, please open an issue in the [Documentation repository](https://github.com/immuni-app/immuni-documentation). This lets everyone involved see it, consider it, and participate in the discussion, without slowing down the overall process.
